...
export class CustomComponent implements OnInit, OnDestroy {
  ...
  formatBoxWrapperWidth: number = 0;
  formatBoxWrapperHeight: number = 0;
  formatBoxWrapperSize: string = "";
  formatBoxMaxWidth: number = 480;
  conversationWidth: number = 320;
  ...
  ngOnInit(): void {
    ...
    this.formatBoxWrapperWidth = this.format.formatWidth;
    this.formatBoxWrapperHeight = this.format.formatHeight - 40;
    this.formatBoxWrapperSize = `(${this.formatBoxWrapperWidth} x ${this.format.formatHeight})` 
    this.conversationWidth = (this.formatBoxMaxWidth <= this.formatBoxWrapperWidth) ? this.conversationWidth : this.formatBoxWrapperWidth;

    const backImgIndex = this.format.formatSpec.resourceSpec.findIndex(ele => ele.name === "background image");
    this.format.formatSpec.resourceSpec[backImgIndex].width = this.format.formatWidth;
    this.format.formatSpec.resourceSpec[backImgIndex].height = this.format.formatHeight;
  }
